...
static uint32_t get_crc(const uint8_t *data, size_t size) {
    ...
    for (size_t i = 0; i < size; i++) {
        crc ^= data[i];
        for (int j = 0; j < 8; j++) crc = (crc >> 1) ^ (-(int)(crc & 1) & 0xEDB88320);
    }
...
static int write_header(zip_archive_t *ar, zip_entry_t *e, int is_cd) {
    ...
    write_u16(ar->fp, 20); write_u16(ar->fp, 0); write_u16(ar->fp, 0); write_u16(ar->fp, 0); write_u16(ar->fp, 0);
    write_u32(ar->fp, e->crc32); write_u32(ar->fp, e->comp_size); write_u32(ar->fp, e->uncomp_size);
    write_u16(ar->fp, e->name_len); write_u16(ar->fp, 0);
...
int zip_add_buffer(zip_archive_t *ar, const void *buf, size_t size, const char *zip_name) {
    if (ar->count >= ar->capacity) {
        ar->entries = realloc(ar->entries, (ar->capacity *= 2) * sizeof(zip_entry_t));
    }
...
int zip_finalize(zip_archive_t *ar) {
    ...
    write_u32(ar->fp, cd_size); write_u32(ar->fp, cd_start); write_u16(ar->fp, 0);
    fclose(ar->fp); free(ar->entries); free(ar); return 0;
}
#endif
